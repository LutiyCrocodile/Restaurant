...
class CreateOrders(tk.Toplevel):
    def __init__(self, parent, func):
        ...
        # Labels
        self.cst_lbl = ttk.LabelFrame(self.main_frame, text="Создать заказ")
        ...
        self.tb_name = ttk.Label(self.cst_lbl, text="Номер стола:")
        self.tb_name.grid(column=1, row=1)

        self.pr_name = ttk.Label(self.cst_lbl, text="Имя продукта")
        self.pr_name.grid(column=0, row=3)

        self.pr_qty = ttk.Label(self.cst_lbl, text="Количество")
        self.pr_qty.grid(column=1, row=3)

        self.pr_st = ttk.Label(self.cst_lbl, text="Статус")
        ...
        self.btn_add_product = ttk.Button(
            self.main_frame, text="Добавить продукт", command=self.add_product)
        self.btn_add_product.grid(column=0, row=2, padx=(50, 0), pady=10)

        self.close_btn = ttk.Button(
            self.main_frame, text='Закрыть', command=self.destroy)
        self.close_btn.grid(column=1, row=2, padx=(50, 0), pady=10)

        self.send_to_ch = ttk.Button(
            self.main_frame, text='Готовить', state=tk.DISABLED, command=self.send_to_kitchen)
    ...
    def callback_table_num(self, P):
        if (str.isdigit(P) and int(P) <= self.fac_info[1]) or P == "":
            ...
            messagebox.showerror(
                "Ошибка ввода", f"Максимальное кол-во столов не должно превышать {self.fac_info[1]}!")
        ...
        try:
            ...
            if table_num:
                for order in self.order_ls:
                    if order.retrieve_data()[0] == 'Выберите блюдо':
                        messagebox.showerror(
                            "Блюдо не выбрано", "Пожалуйста, либо заполните \"Выберите блюдо\", либо удалите эту строку, чтобы продолжить!")
                        return False
                    else:
                        orders.append((order.retrieve_data(), table_num))
                self.add_records(orders)
                usr_resp = messagebox.askyesno(
                    "Успешно", "Заказ готовится, желаете заказать еще?")
                if usr_resp:
                    self.clear()
                else:
                    self.destroy()
                ...
                messagebox.showerror(
                    "Пустые значения", "Пожалуйста, введите корректое кол-во столов!")
        except Error as e:
            print(e)
