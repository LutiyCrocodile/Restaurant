...
class ConfigWindow(tk.Toplevel):
    def __init__(self, parent, func):
        ...
        self.title('Система управления рестораном')
        ...
        self.fac_config_lf = ttk.LabelFrame(
            self.up_frame, text="Редактирование Ресторана")
        ...
        # Labels
        self.fc_name_lb = ttk.Label(self.fac_config_lf, text="Имя Объекта:")
        ...
        self.fc_save_btn = ttk.Button(
            self.fac_config_lf, text="Сохранить", command=self.save_fac_config)
        ...
        self.menu_conf_lf = ttk.LabelFrame(
            self.down_frame, text="Редактирование меню")
        ...
        self.tr_view.column('#', width=50, anchor=tk.CENTER)
        self.tr_view.column('Имя', width=200, anchor=tk.CENTER)
        self.tr_view.column('Цена', width=200, anchor=tk.CENTER)

        self.tr_view.heading('#', text="№")
        self.tr_view.heading('Имя', text="Имя")
        self.tr_view.heading('Цена', text="Цена(р)")
        ...
        self.pr_id_lbl = ttk.Label(
            self.remove_prd_lbf, text="Выбранный продукт:")
        ...
        self.tr_view_add = ttk.Button(
            self.add_prd_lbf, text="Добавить", command=self.add_record)
    ...
    def callback_table(self, P):
        if (str.isdigit(P) and int(P) <= 50) or P == "":
            ...
            messagebox.showerror(
                "Некорректный ввод", "Максимальное число столов не должно превышать 50!")
            return False
    ...
    def callback_seats(self, P):
        ...
        if (str.isdigit(P) and int(P) <= max_table_num) or P == "":
            ...
            messagebox.showerror(
                "Некорректный ввод", f"Максимальное число посадочных мест не должно превышать {max_table_num}.")
            return False
    ...
    def validate_product(self, price, name):
        if (self.is_float(price) and float(price) <= 10000000) and (len(name) <= 20):
            ...
            usr_resp = messagebox.askyesno(
                "Высокая цена", "Цена выше 10 миллионов, вы уверены, что хотите продолжить?")
            if usr_resp:
                return True
            else:
            ...
            usr_resp = messagebox.showerror(
                "Неправильный ввод", "Цена выше 10 миллионов и имя продукта содержит более 20 знаков.")
            self.food_price_entry.delete(0, tk.END)
            self.food_name_entry.delete(0, tk.END)
            return False
        else:
            messagebox.showerror(
                "Неправильный ввод", "Пожалуйста, введите имя продукта (не превышающее 20 знаков) и цену (не превышающую 10 млн.) правильно.")
            return False
    ...
    def save_fac_config(self):
        ...
        if fac_name and table_num and seat_num:
            ...
            messagebox.showerror(
                "Пустые значения", "Пожалуйста, введите имя объекта, номер стола и номер посадочного места соответственно.")
    ...
    def add_record(self, event='<Return>'):
        ...
        if food_name and food_price:
            ...
            er_msg = "Пожалуйста, заполните: \"Имя продукта \" и \"Цена продукта\""
            messagebox.showerror("Пустые значения", er_msg)
