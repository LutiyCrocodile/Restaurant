...
class OrderedProducts(tk.Frame):
    def __init__(self, parent, root_frame, label_frame, table_num, func):
        ...
        self.table_num = f"Стол {table_num}"
        ...
        # TreeView
        self.tr_view_columns = ('Имя', 'Количество', 'Статус')
        ...
        self.tr_view.column('Количество', width=100, anchor=tk.CENTER)
        self.tr_view.column('Статус', width=200, anchor=tk.CENTER)

        self.tr_view.heading('Имя', text="Имя продукта")
        self.tr_view.heading('Количество', text="Количество")
        self.tr_view.heading('Статус', text="Статус")
        ...
        self.cooked_btn = ttk.Button(
            self.tb, text=f"Приготовлено", command=self.change_state)
        self.cooked_btn.grid(column=1, row=7, padx=(0, 200), pady=10)
        self.cooked_btn.config(state='disabled')

        self.flf_btn = ttk.Button(
            self.tb, text="Выполнить заказ", command=self.fulfil_order, state=tk.DISABLED)
    ...
    def check_for_cooked(self):
        ...
        btn_state = tk.DISABLED if "Заказан" in or_stat else tk.ACTIVE
    ...
    def update_order_db(self):
        try:
            ...
            self.fac_db.delete_val(delete_query, ["Приготовлен"])
        except Error as e:
            print(e)
    ...
    def change_state(self):
        ...
        self.tr_view.item(sel_item, text="", values=(
            retrieved_value[0], retrieved_value[1], "Приготовлен"))
